...
<div class="card">
  ...
  <div class="table-wrap">
    <table id="transTable">
      ...
      <tbody id="transBody">
        <tr>
          <td colspan="6" style="text-align:center;padding:32px;color:var(--text-muted)">⏳ Ładowanie…</td>
        </tr>
      </tbody>
    </table>
  </div>
...
<script>
  ...
  async function loadCategories() {
    try {
      ...
      toast('Błąd ładowania kategorii.', 'error');
    }
  }
  ...
  async function loadTrans() {
    try {
      ...
      <tr>
        ...
        <td>
          ${r.category_name && r.category_name !== 'Brak' && r.category_name !== 'Inne'
          ...
          : '<span style="color:var(--text-muted)">— brak kategorii —</span>'}
        </td>
    ...
    `).join('');
      ...
      toast('Błąd ładowania transakcji.', 'error');
    }
  }
  ...
  // ── Modal ──────────────────────────────────────────────────
  async function openModal(id = null, data = null) {
    editingId = id;
    ...
    const typeEl = document.getElementById(type === 'income' ? 'typeIncome' : 'typeExpense');
  ...
  document.getElementById('saveModal').addEventListener('click', async () => {
    ...
    if (!typeEl) { toast('Wybierz typ transakcji.', 'error'); return; }
    ...
    if (!payload.amount || !payload.date) { toast('Uzupełnij wymagane pola (kwota i data).', 'error'); return; }
    ...
    try {
      ...
      toast('Błąd podczas zapisywania: ' + e.message, 'error');
    }
  });
